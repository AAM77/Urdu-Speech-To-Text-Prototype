...
_ARTICLE_SCHEMA_INSTRUCTIONS = (
    "Return JSON with exactly these keys and no extra prose: "
    "`title` (string), `subtitle` (string or null), and "
...
def _render_instructions(template: str) -> str:
    return template.replace(
        "{{SOURCE_TEXT}}",
        "[English translation source data is supplied separately.]",
    )
...
class ArticleGeneratorStage:
    def __init__(
        ...
        *,
        ...
        if store is None and artifact_sink is None:
            raise ValueError(
                "ArticleGeneratorStage requires an ArtifactStore or ArtifactSink."
            )
        ...
        if self.cache_store is not None and self.cache_scope is None:
            ...
            self.cache_scope = CacheScope(user_id=user_id, name="article_generator")
        self.budget_guard = budget_guard
        self.provider_registry = provider_registry
        self.provider_config = (
            provider_registry.get_active_config() if provider_registry is not None else None
        )
        fallback_model_id = get_model_roles(self.settings).for_role("article")
        self.model_id = (
            self.provider_config.model_roles.get("article", fallback_model_id)
            if self.provider_config is not None
            else fallback_model_id
        )
        self.prompt_version = (
            self.provider_config.prompt_versions.get("article", self.settings.prompt_version)
            if self.provider_config is not None
    ...
    def run(self, translation: EnglishTranslationArtifact) -> ArticleArtifact:
        ...
        if cache_hit and cached_payload:
            article = Article.model_validate(cached_payload["article"])
        else:
            request = TextGenerationRequest(
                model_id=self.model_id,
                developer_instructions=_render_instructions(self._template),
                schema_instructions=_ARTICLE_SCHEMA_INSTRUCTIONS,
                source_data=ProviderSourceData.from_text(
                    source,
                    metadata={
                        "source_artifact_id": translation.manifest.artifact_id,
                        "source_language": "en",
                    },
                ),
        ...
        self.artifact_sink.write_artifact(artifact, "final_article.json")
        self.artifact_sink.write_markdown(_to_markdown(article), "final_article.md")
        return artifact
